...
selected_tags_over_time %>%
  ...
  labs(
    title = "CHANGE OF R, DPLYR AND GGPLOT2 TAGS OVER TIME") +
    theme(panel.background = element_rect(fill = "GhostWhite") )
```

<p style="color:blue"><i> R has grown quite rapidly over the past decade while the dplyr and ggplot2 tags are increasing steadily as well.</i></p>
...
by_highest_tag_subset %>% 
  ggplot(aes(year,fraction,color = tag)) + geom_line() +
  labs(
    title = "TOP 6 HIGHEST TAGS OVER TIME") +
    theme(panel.background = element_rect(fill = "GhostWhite") )
```
...
<p style="color:blue"><i> In the three main mobile operating systems (Android, iOS and Windows Phone), the Windows Phone tag has fewest question, whereas the two remaining tags have been increasing a fraction of questions pretty fast in the last decade.</i></p>
...
<p style="color:blue"><i> Since 2014, the popularity of languages like Javascript and Java are still the most, whereas
the number of people using c# and html is shrinking.</i></p>
